#define FONT_SIZE 16 // Adjust the font size as needed
#define MESSAGE_BOX_PADDING_X 2 // Padding for message box (left and right)
...
void drawtilesBox(SDL_Renderer *renderer, const char* fontPath, int fontSize, const char* message, float x, float y, int maxheight,int textWidth) {
    if (TTF_Init() == -1) { // Initialize SDL_ttf
        printf("SDL_ttf could not initialize! SDL_ttf Error: %s\n", TTF_GetError());
        return;
    }
    ...
    const char* end = message;
  // Draw message box with rounded corners
    SDL_Rect messageRect = { x - MESSAGE_BOX_PADDING_X/3, y - MESSAGE_BOX_PADDING_Y/2, textWidth + 2* MESSAGE_BOX_PADDING_X, maxheight + 3 * MESSAGE_BOX_PADDING_Y };
    drawRoundedRect(renderer, messageRect, 24, messageBoxColor);
    ...
    while (*end) {
        ...
        // Render the line
        SDL_Surface* textSurface = TTF_RenderText_Blended_Wrapped(font, start, textColor, textWidth);
        if (textSurface == NULL) {
            printf("Unable to render text surface! SDL_ttf Error: %s\n", TTF_GetError());
            TTF_CloseFont(font);
            TTF_Quit();
        ...
        // Render text texture
        SDL_Rect textRect = { x + MESSAGE_BOX_PADDING_X, textY + 2*MESSAGE_BOX_PADDING_Y, textSurface->w, textSurface->h };
        SDL_RenderCopy(renderer, textTexture, NULL, &textRect);
